          ...
          <div class="row">
            <div class="col-12">
              <table class="table table-striped table-bordered">
                <h2> Yaz Okulu Başvuru Yükleme Alanı</h2>
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <div>
                        Upload Files<br />
                        <form method="POST" id="uploadForm" action="{{ route('upload.store', ['yazokulu']) }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
...
<!--
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="{{ route('upload.store', ['yazokulu']) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}

                        <input type="file" name="yazokuluPdf">
                        <input type="submit">
                      </form>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>Çap Başvurusu</td>
                    <td>
                      <form method="POST" action="{{ route('upload.capstore', ['cap']) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}

                        <input type="file" name="capPdf">
                        <input type="submit">
                      </form>
                    </td>
                  ...
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="{{ route('upload.dgsstore', ['dgs']) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}

                        <input type="file" name="dgsPdf">
                        <input type="submit">
                      </form>
                  ...
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="{{ route('upload.yataystore', ['yatay']) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}

                        <input type="file" name="yatayPdf">
                        <input type="submit">
                      </form>
                  ...
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="{{ route('upload.dersintibakstore', ['dersintibak']) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}

                        <input type="file" name="dersintibakPdf">
                        <input type="submit">
                      </form>
  ...
  <script>
    ...
    document.getElementById("send").addEventListener("click", function() {
      ...
      if (files.length != 0) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          var storage = firebase.storage().ref('pdf/yazokulu/' + files[i].name);
    ...
    document.getElementById("send").addEventListener("click", function() {
      ...
      if (files.length != 0) {
        ...
        for (let i = 0; i < files.length; i++) {
          //create a storage reference
          var database = firebase.database().ref('pdf/yazokulu/' + files[i].name);
