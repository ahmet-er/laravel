          ...
          <div class="row">
            <div class="col-12">
              <table class="table table-striped table-bordered">
                <h2> Ders İntibak Başvuru Yükleme Alanı</h2>
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <div>
                        Upload Files<br />
                        <form method="POST" id="uploadForm" action="{{ route('upload.store', ['dersintibak']) }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
  ...
  <script>
    ...
    document.getElementById("send").addEventListener("click", function() {
      ...
      if (files.length != 0) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          var storage = firebase.storage().ref('pdf/dersintibak/' + files[i].name);
    ...
    document.getElementById("send").addEventListener("click", function() {
      ...
      if (files.length != 0) {
        ...
        for (let i = 0; i < files.length; i++) {
          //create a storage reference
          var database = firebase.database().ref('pdf/dersintibak/' + files[i].name);
